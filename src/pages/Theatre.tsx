...
const Theatre = () => {
  ...
  const handleSearch = (text: string) => {
    ...
    const filteredData = etkinlikler.filter(
      (item) =>
        item.Tur === 'TİYATRO' &&
        item.Adi.toLowerCase().includes(lowercaseText)
    );
            ...
            renderItem={({ item }) => (
              <View style={StyleDesign.itemContainer}>
                <Image
                  ...
                  style={StyleDesign.image}
                />
                <View style={StyleDesign.infoContainer}>
                  <ScrollView>
                    <Text style={StyleDesign.eventName}>{item.Adi}</Text>
                    <Text style={StyleDesign.eventDate}>
                      {item.EtkinlikBaslamaTarihi}
                    </Text>
                    <Text style={StyleDesign.eventLocation}>
                      {item.EtkinlikMerkezi}
                    </Text>
                    <Text style={StyleDesign.eventType}>
                      {item.Tur.charAt(0).toUpperCase() +
                        item.Tur.slice(1).toLowerCase()}
                    </Text>
                    <Text style={StyleDesign.eventFree}>
                ...
                <View style={StyleDesign.buttonContainer}>
                  <View style={StyleDesign.buttonContainerArea}>
                    <TouchableOpacity
                      style={StyleDesign.detailLocationButton}
                      onPress={() => openEventUrl(item.EtkinlikUrl)}
                    >
                  ...
                  <View style={StyleDesign.buttonContainerArea}>
                    <TouchableOpacity
                      style={StyleDesign.detailLocationButton}
                      onPress={() =>
                        Linking.openURL(item.EtkinlikMerkeziKonum)
                      }
                    >
